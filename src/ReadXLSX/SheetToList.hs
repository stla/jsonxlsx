...
extractColumn fcells fcellToValue skip j =
  DV.fromList $
    map (\i ->
...
sheetToMap :: FormattedCellMap -> (FormattedCell -> Value) -> Bool -> Bool ->
                InsOrdHashMap Text Array
sheetToMap fcells fcellToValue header fixheaders =
  DHSI.fromList $
    map (\j ->
          (colnames !! j, extractColumn fcells fcellToValue skip (j+firstCol)))
        [0 .. length colnames - 1]
  where (skip, colnames) = if header
                             then
                               (1, colHeaders2 fcells fixheaders)
                             else
...
sheetToMapMap fcells header fixheaders =
  DM.map
    (\valueGetter -> DHSI.fromList $
      map (\j ->
            (colnames !! j, extractColumn fcells valueGetter skip (j+firstCol)))
        [0 .. length colnames - 1])
  where (skip, colnames) = if header
                              then
                                (1, colHeaders2 fcells fixheaders)
                                ...
                                (0, map (\j ->
                                          T.concat [x, showInt j]) colRange)
        (colRange, firstCol, _) = cellsRange fcells
        x = pack "X"

-- | Read several sheets
--  sheetsToMapMap (Map sheetName -> FormattedCell) header (Map what -> value getter)
sheetsToMapMap :: Map Text FormattedCellMap -> Bool -> Bool ->
                    Map Text (FormattedCell -> Value) ->
                      Map Text (Map Text (InsOrdHashMap Text Array))
sheetsToMapMap fcellmapmap header fixheaders gettermap =
  DM.map (\fcellmap ->
            sheetToMapMap fcellmap header fixheaders gettermap) fcellmapmap
